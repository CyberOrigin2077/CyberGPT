# CyberGPT: Cyber World Model Challenge

The robotic world model need large amount of dataset. World model has long been a hot topic for the robotics research. Even though there are different kinds of datasets for manipulation operations, such dataset is mainly restricted on the specific robotic platforms.

In contrast, CyberGPT treated this task from the "First princinple theory", where all the robotics are actually learning skills from human operators. Different from the exisiting world model in Robotics, CyberGPT provides the first real-world operation system for diverse challeging environments.

Progress in the recent video generation may soom make it possible to evaluate robot policies in a completely learned world model. An end-to-end learned simulator (World Simulator) of millions of human operations in the real world environments would greatly accelerate progress in the general-purpose robotics and provide a useful signal for scaling data and compute. To boost the progress in the learned world simulator for general-purpose AI, we're announcing the Cyber World Model, where the task is to predict future first-person observations of the [Human Operators](https://www.1x.tech/androids/eve).

We provide over 100 hours of vector-quantized image tokens and raw actions collected from operating EVE at 1X offices, baseline world model (GENIE-style), and a frame-level MAGVIT2 autoencoder that compresses images into 16x16 tokens and decodes them back into images.
...
- **Annotation Challenge (\$10k prize)**: Image and Video Clip annotation based on the exist action $a_t$, observation $o_t$, $p_i(z_t|o_t,a_t)$.
